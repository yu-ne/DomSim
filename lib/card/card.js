...
var Shepherd = function() {
  Card.call(this, CardDb[Card.NAME.SHEPHERD]);
  ...
  this.play = function(player) {
    ...
    // 手札の勝利点カードをすべて捨て札にする。
    var cardNames = player.hand.getCardsByCardType(Card.TYPE.VICTORY).getCardNames();
    if(cardNames.length === 0) {
      return;
    }
    player.discardMulti(cardNames);

    // 捨てた枚数*2だけドロー
    player.draw(cardNames.length * 2);
  }
}
 ...
 */
var Souk = function() {
  Card.call(this, CardDb[Card.NAME.SOUK]);
  ...
  this.play = function(player) {
    ...
    player.addCoin(7);
    // 手札1枚につき、-1コイン
    player.addCoin(-player.hand.count());
  }
}