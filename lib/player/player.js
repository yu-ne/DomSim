...
var Player = function(name, ai) {
   ...
   * カードを獲得する。
   * @param {!Card} card カード
   * @param {!Player.GAIN_TO} gainTo 獲得先(省略可能)
   */
  this.gain = function(card, gainTo) {
    if(typeof gainTo === 'undefined') {
      gainTo = Player.GAIN_TO.DISCARD;
    }

    this.playLogger.log("gains", card.name);
    switch(gainTo) {
    case Player.GAIN_TO.DISCARD:
      this.discardPile.push(card);
      break;
    case Player.GAIN_TO.HAND:
      this.hand.push(card);
      break;
    default:
      // 処理なし
   ...
   * @param {!Player.GAIN_TO} gainTo 獲得先(省略可能)
   */
  this.gainByCardName = function(cardName, gainTo) {
    if(typeof gainTo === 'undefined') {
      gainTo = Player.GAIN_TO.DISCARD;
    }
    var card = board.supply.popSupplyCard(cardName);
    this.gain(card, gainTo);
  }
   ...
   * @param {!string} cardName カード名
   * @param {!Player.GAIN_TO} gainTo 獲得先(省略可能)
   * @returns 獲得した場合true、それ以外の場合false
   */
  this.tryGain = function(cardName, gainTo) {
    if(typeof gainTo === 'undefined') {
      gainTo = Player.GAIN_TO.DISCARD;
    }
    if(this.canGain(cardName) === false) {
      return false;
    }
    this.gainByCardName(cardName, gainTo);
    return true;
  }
   ...
   * 指定カードを獲得可能か判定する。
   * @param {!string} cardName カード名
   * @returns 獲得可能な場合true、そうでない場合false
   */
  this.canGain = function(cardName) {
    return !board.supply.isEmpty(cardName);
  }
   ...
   * @param {!string} cardName
   */
  this.discard = function(cardName) {
    this.discardMulti([cardName]);
  }
   ...
   * @param {!string} cardName
   */
  this.discardMulti = function(cardNames) {

    this.playLogger.log("discards", cardNames.join(", "));
    var self = this;
    cardNames.each(function(cardName) {
      var card = self.hand.popByName(cardName);
      if(card !== null) {
